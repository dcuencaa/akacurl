...
Aka Curl is a cross-platform, graphical user interface (GUI) tool designed specifically for testing and troubleshooting Akamai CDN edge environments. It acts as an advanced wrapper around native OS commands (`curl` and `openssl`) to bypass common Python SSL/SNI limitations when spoofing IP addresses or testing Enhanced TLS configurations.
...
* **Context-Aware Search:** Press `Ctrl+F` (or `Cmd+F` on macOS) to instantly search through thousands of lines of headers, JSON bodies, or certificate outputs with real-time highlighting.
...
* **Modern UI:** Clean, flattened interface using the "Cove" color palette, offering a native Akamai-like look and feel.


____________________________________________________________________________________________________________
# Aka Curl - Installation Guide

This guide covers the prerequisites and step-by-step installation instructions for getting **Aka Curl** running smoothly on macOS, Linux, and Windows.
...
Welcome to the **Aka Curl** User Guide. This tool simplifies the process of testing Akamai edge configurations, spoofing Edge IPs, validating SSL/TLS handshakes, and debugging responses.
...
* **Provide a Map:** (e.g., `e19.dscg.akamaiedge-staging.net`) Overrides the domain's normal map manually.

### Network & Env
This dropdown tells Aka Curl how to route the request if Auto-CNAME is used, or which default fallback map to apply if no CNAME is found.
...
* **Version:** Defaults to `HTTP/1.1`. You can test `HTTP/2` or `HTTP/3` (Note: HTTP/3 requires a local `curl` binary compiled with experimental HTTP/3 support).

---

## 2. Toggles & Options

* **Pragma Headers:** Injects standard Akamai debug headers into your request to expose Cache statuses (`TCP_HIT`, `TCP_MISS`), CP Codes, and Cache Keys.
* **Save/Show Body:** * If the response is readable (JSON, HTML, text), it will display in the "Response Body" tab.
  * If the response is a binary file (PDF, image, video), the app will prompt you with a native "Save As" dialog to safely download the file to your computer.
...
### Tab 4: Decoded Cert
Takes the raw PEM certificate block from Tab 3 and decodes it using `openssl x509`. This provides a highly readable view of the certificate's **Expiration Date**, **Issuer**, and **Subject Alternative Names (SANs)** without needing to manually copy/paste the block into a decoder.
...
Aka Curl features a powerful, context-aware search bar to help you dig through thousands of lines of output.
